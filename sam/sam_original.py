...
'''
def sam_original(prompt_type, img_folder, prompt_folder, output_folder, cdkpt_path, img_num=10, is_demo=True, file_extension=".png"):
    """
    ...
    for filename in sorted_filelist:
        if filename.endswith(file_extension):  # only operate image file
            ...
            # Compare the image size between the original and the prompt
            if img.size != prompt.size:
                # Resize the prompt image to match the original image's size, maintaining aspect ratio
                prompt = ImageOps.fit(prompt, img.size, Image.Resampling.LANCZOS)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-img", help="Path to the orginal images (Ultrasound image) folder.")
    parser.add_argument("-prompt", help="Path to the masks (ground truth or generated by U-Net) folder.")
    parser.add_argument("-output", help="Path to the output folder where matched files will be copied.")
    parser.add_argument("-ckpt", help="Path to the checkpoint file.")
    parser.add_argument("-b", default="10", help="Number or the batch size")
    parser.add_argument("-isdemo", default=True, help="True means this run is for testing or presenting, will just run a part of the dataset (10 images)")
    parser.add_argument("-extension", default=".png", help="File extension to match (default: '.png').")

    args = parser.parse_args()

    sam_original(args.prompt_type, args.img, args.prompt, args.output, args.ckpt, args.b, args.isdemo, args.extension)
